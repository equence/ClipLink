    ...
    : QMainWindow(parent), m_deviceName(std::move(deviceName))
{
    ...
    connect(settingsButton, &QToolButton::clicked, this, [this]{ bool ok=false; const QString name=QInputDialog::getText(this,"ClipLink 设置","设备名称",QLineEdit::Normal,m_deviceName,&ok); if(!ok || name.trimmed().isEmpty()) return; const QString host=QInputDialog::getText(this,"ClipLink 设置","默认服务器",QLineEdit::Normal,"127.0.0.1",&ok); if(!ok) return; const int port=QInputDialog::getInt(this,"ClipLink 设置","默认端口",45454,1,65535,1,&ok); if(ok){m_deviceName=name.trimmed(); emit settingsChanged(m_deviceName,host,static_cast<quint16>(port));}});
}
